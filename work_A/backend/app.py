...
# 失物分类配置（与前端保持一致）
ALLOWED_CATEGORIES = ["图书馆", "食堂", "教学楼", "操场", "宿舍", "其他"]
DEFAULT_CATEGORY = "其他"
# 状态白名单（用于列表筛选）
ALLOWED_STATUS = ["未认领", "已认领"]
...
def item_to_dict(it, publisher_student_verified=False):
    ...
    return {
        ...
        "category": getattr(it, "category", None) or "其他",
...
def list_categories():
    """返回可用分类列表，前端打开页面时可拉取。"""
    return jsonify({"code": 0, "message": "ok", "data": ALLOWED_CATEGORIES})
...
def list_items():
    """
    获取失物列表，支持以下可选查询参数（多条件 AND 叠加）：
      - keyword:  关键词，在 title / description 中做不区分大小写的子串匹配
      - category: 分类，必须在 ALLOWED_CATEGORIES 里，否则忽略（= 不筛选）
      - status:   状态，"未认领" 或 "已认领"，其它值忽略（= 不筛选）
    """
    keyword = (request.args.get("keyword") or "").strip().lower()
    category = (request.args.get("category") or "").strip()
    status = (request.args.get("status") or "").strip()

    query = Item.query.filter_by(is_draft=False)
    ...
    # 关键词筛选（Python 层面，支持中文子串匹配）
    if keyword:
        rows = [
            it for it in rows
            if keyword in (it.title or "").lower()
            or keyword in (it.description or "").lower()
        ]
...
def update_item(item_id):
    ...
    category = (data.get("category") or "").strip()
    if category not in ALLOWED_CATEGORIES:
        category = getattr(item, "category", None) or DEFAULT_CATEGORY
    ...
    item.category = category  # 【修复】保存分类
    db.session.commit()
...
def publish_draft(item_id):
    ...
    item.publish_latitude = plat
    item.publish_longitude = plng
    item.category = category  # 【修复】发布草稿时保存分类
    item.is_draft = False
    db.session.commit()
...
def _ensure_category_column():
    """为旧数据库补充 category 字段（旧数据自动设为"其他"）。"""
    insp = inspect(db.engine)
    if not insp.has_table("items"):
        return
    names = {c["name"] for c in insp.get_columns("items")}
    if "category" not in names:
        db.session.execute(
            text("ALTER TABLE items ADD COLUMN category VARCHAR(20) NOT NULL DEFAULT '其他'")
        )
        db.session.commit()
